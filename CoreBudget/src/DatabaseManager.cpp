...
void DatabaseManager::saveWallet(const Wallet &wallet,
  ...
  // === Szyfrowanie oraz zapis i odczyt pliku binarnego ===
...
void DatabaseManager::loadWallet(Wallet &wallet, const std::string &filename,
                                 const std::string &password) {
  ...
  auto readString = [&buffer, &index, size]() -> std::string {
    if (index + sizeof(size_t) > static_cast<size_t>(size))
      throw DatabaseException(
          "Bledne haslo lub plik uszkodzony (rozmiar tekstu).");
    size_t length;
    ...
    if (index + length > static_cast<size_t>(size))
      throw DatabaseException(
          "Bledne haslo lub plik uszkodzony (zawartosc tekstu).");
  ...
  for (size_t i = 0; i < txCount; ++i) {
    ...
    if (type == 'T') {
      ...
      if (index >= static_cast<size_t>(size))
        throw DatabaseException(
            "Plik jest uszkodzony (brak flagi cyklicznej)!");
      bool recurring = (static_cast<uint8_t>(buffer[index++]) != 0);

      std::string accountName = readString();

      if (type == 'I') {
        wallet.addTransaction(std::make_unique<Income>(amount, cat, date,
                                                       recurring, accountName));
      } else if (type == 'E') {
        wallet.addTransaction(std::make_unique<Expense>(
            amount, cat, date, recurring, accountName));
      } else {
        throw DatabaseException(
            "Nieznany typ transakcji. Prawdopodobnie bledne haslo!");
      }
    }
  }
  ...
  // else: monthlyBudgetLimit pozostaje domyślne 5000.0
